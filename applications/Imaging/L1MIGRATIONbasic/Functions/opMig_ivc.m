...
%
    ...
    methods
       ...
       % Constructor
       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       function op = opMig_ivc(U,R,freq,nx,nz,nx_border,nz_border,xd,zd,vel)
		...
		fun = @(x,mode) opMig_intrnl(U,R,freq,n,ns,nf,nx,nz,nx_border,nz_border,xd,zd,sigma,vel,x,mode);

          % Construct operator
          op = op@opSpot('Mig&DeMig',nx*ns*nf,nx*nz);
          op.cflag     = 1;
          op.funHandle = fun;
       end % Constructor
...
function y = opMig_intrnl(U,R,freq,n,ns,nf,nx,nz,nx_border,nz_border,xd,zd,sigma,vel,x,mode) 
...
elseif (mode == 2)   % migration mode
    ...
    parfor ifreq = 1:nf
        ...
        A          = opHelm2D9pt(vel,xd,zd,nx_border,nz_border,f);       
        omg        = -2*(2*pi*f).^2;
        y1         = omg.*conj(U(:,:,ifreq)).*(A'\xidex);
        y(:,ifreq) = .0001*sum(y1,2);
    end
    y              = sum(y,2);
