...
snapshot_file = [resultdir,'/MigTotalFD_iwave_26ss31freq_EstQ_GausEnc_wAmpCor_IG2_BS_snapshots.mat'];
result_file = [resultdir,'/MigTotalFD_iwave_26ss31freq_EstQ_GausEnc_wAmpCor_IG2_BS.mat'];
...
freq_lower = df;
freq_upper = 38;
freq_lower = df*round(freq_lower/df);
freq_upper = df*round(freq_upper/df);
freq = freq_lower:df:freq_upper;
...
% border-trimming op
opBorderMute = opTrimBorder(nz, nx, model.nb(1), model.nb(2));
...
inv_para.opRight = opBorderMute*opModelMute*opDepth*opGetReal*C;
...
global S_f_full fidx

% initial guess of the wavelet
S_f_full = freq.^2.*exp(-(freq/15).^2).*exp(1i*2*pi* freq*(-0.04));
s_full = opFT1D'*S_f_full(:);
s_full_rot = hilbert(s_full);
s_full_rot = imag(s_full_rot);
S_f_full = opFT1D*s_full_rot;
S_f_full = transpose(S_f_full);
S_f_full = S_f_full/max(abs(S_f_full));

[x,info] = Mig_SRM_IG(m0, Q, D, model, inv_para);
dm = inv_para.opRight*x;
dm = reshape(dm, nz, nx);
dm = dm(model.np(1)+1:end,:);
save(result_file,'x','dm','info','S_f_full','fidx')
