...
data_file = [datadir,'/LinearData.mat'];
model_file = [datadir,'/sigsbee_nosalt_model.mat'];
source_file = [datadir,'/preproc_FDdata.mat'];
snapshot_file = [resultdir,'/MigTotalLinear_26ss31freq_EstQ_GausEnc_snapshots.mat'];
result_file = [resultdir,'/MigTotalLinear_26ss31freq_EstQ_GausEnc.mat'];
...
scale_op = 1/1000;
load(data_file,'dP');
dP = vec(distributed(dP));
D.Pr = scale_op*dP;
load(source_file,'P_up_SURF')
P = vec(distributed(P_up_SURF));
mul_scale = 1/25;
D.Ps = -mul_scale*P;

clear dP P P_up_SURF
...
save(result_file,'x','dm','info','S_f_full','S_f','fidx')
curvelet_denoise(result_file, 0.001)