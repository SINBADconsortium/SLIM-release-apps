function f = projMigSRM_IG(x, g, params)
...
% compute frequency spectrum of source
S_f_correction = ones(size(S_f));
dm = opRight*x;
if not(all(not(dm)))
    pri_mod = invvec(pri_mod,[nrec*nsrc,nf]);
    res = invvec(b-mul_mod,[nrec*nsrc,nf]);
    for i=1:nf
        S_f_correction(i) = gather(pri_mod(:,i)'*res(:,i)/(pri_mod(:,i)'*pri_mod(:,i)));
    end
    S_f = S_f.*S_f_correction;
    S_f_full(fidx) = S_f;
    pri_mod = pri_mod(:);
end
...
opConvS_correction = oppKron2Lo(opDiag(S_f_correction),opDirac(nrec*nsrc));
if isempty(g)
    f = opConvS_correction*pri_mod+mul_mod;
else
    f = A'*g;
end