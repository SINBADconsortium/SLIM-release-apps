...
snapshot_file = 'MigTotalFD_iwave_26ss31freq_bg_TrueQ_GaussianEnc_IgnoreMul_snapshots.mat';
result_file = 'MigTotalFD_iwave_26ss31freq_bg_TrueQ_GaussianEnc_IgnoreMul.mat';
...
[x,info] = Mig_Pri(m0, Q, D.Pr, model, inv_para);
dm = inv_para.opRight*x;
dm = reshape(dm, nz, nx);
dm = dm(model.np(1)+1:end,:);
save(result_file,'x','dm','info')
curvelet_denoise(result_file, 0.01)