function [opPrecon, opSub, dm_deconv, norm_opPrecon] = est_opPrecon_norm(vel_bg, data, model_para, source, options)
% Syntax:
% [opPrecon, dm_deconv, norm_opPrecon] = est_opPrecon_norm(vel_bg, data, model_para, source, options)
% 
% Descriptions:
% Build the deconvolutional imaging preconditioning operator. This preconditioner
% approximates the action of opMig', scaled by frequency/source dependent weigh-
% ting. So the forward mode of this operator applied on the seismic data will give
% you the deconvolutional image.
% 
% Input list:
...
switch options.source_type
  ...
  case 4			% point and areal sources: wavelet by EPSI and total data
    disp('Making preconditioner for total data.')
    sourceQ = opSub*source.Q;
    sourceP = opSub*source.P;
    sourceQmP = sourceQ-sourceP;
    Q = sourcegrid(ns_grid, nss, options.src_polarity, sourceQmP);
end
opPrecon = oppDF_precon(m_bg(:), Q, model_para);

if nargout > 2
    data = opSub*data;
    dm_deconv = opPrecon*data;
    dm_deconv = reshape(dm_deconv, model_para.n);
    if nargout > 3
        data_scale = opPrecon'*dm_deconv(:);
        norm_opPrecon = sqrt(norm(data_scale,1)/norm(data,1));
    end
end