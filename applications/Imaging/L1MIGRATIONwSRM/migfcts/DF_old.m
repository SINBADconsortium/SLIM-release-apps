function output = DF_old(m,Q,input,flag,model)
% Frequency domain modeling in the Born approximation. This is the
% Jacobian of F_old(m,Q,model). 
%
% use: 
%   output = DF_old(m,Q,input,flag,model,{gather})
% input:
...
% dm = randn(size(m));
% dD = DF_old(m,Q,dm,1,model);
% dDt = 0*dD + randn(size(dD));
% dmt = DF_old(m,Q,dDt,-1,model);
% real(dD'*dDt)
% dm'*dmt
%%
