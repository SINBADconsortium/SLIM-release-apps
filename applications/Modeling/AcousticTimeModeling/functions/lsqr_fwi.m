function [ x, istop, itn, r1norm, r2norm, Anorm, Acond, Arnorm, xnorm, var ]...
   = lsqr_fwi( A, b, itnlim, fid, ptol , atol , damp, btol, conlim, show )
[m,n] = size(A);
% Output modified lsqr for Fast-FWI
%  [ x, istop, itn, r1norm, r2norm, Anorm, Acond, Arnorm, xnorm, var ]...
%  = lsqrSOL(A, b, itnlim, fid, damp, atol, btol, conlim, show);
%   fid: fprintf file id
%   ptol: percentage of the torlarence, ||x||_2  < ptol * ||b||_2
% 
%        [ x, istop, itn, r1norm, r2norm, Anorm, Acond, Arnorm, xnorm, var ]...
%  = lsqrSOL(A, b, itnlim, fid, damp, atol, btol, conlim, show);
%
...
%-----------------------------------------------------------------------
if nargin<10,show   = 0;end
if nargin<9,conlim = 1.0e+10;end
if nargin<8,btol   = 1.0e-6;end
if nargin<7,damp   = 0     ;end
if nargin<6,atol   = 1.0e-6;end
if nargin<5,ptol   = 0     ;end
...
% ====== modified part ======
init_norm = r1norm;
fprintf(fid,'%s %s\n', repmat(' ',1,3),repmat('-',1,55));
fprintf(fid,'%s %s\n', repmat(' ',1,10), ' LSQR subproblem convergence log ');
fprintf(fid,'%s %s\n', repmat(' ',1,3),repmat('-',1,55));
fprintf(fid,'%10s  %10s %10s %10s  %10s\n','Iter','Residual','Precent','xNorm(1)','xNorm(2)');
fprintf(fid,' %8g  %10.6e %8.2f%% %10.5f %10.5f\n',itn,r1norm,100,norm(x,1),norm(x,2));
...
norm1 = norm(x,1);
norm2 = norm(x,2);
res_pct = 100*r1norm/init_norm;
if res_pct < ptol
	fprintf(fid,'%s %s %s %s\n',repmat(' ',1,3),repmat('-',1,15),'lsqr subproblem finished',repmat('-',1,15));
	fprintf(fid,'%s %s\n',repmat(' ',1,10),'residule reachs tolerance');
	fprintf(fid,'%s %s\n',repmat(' ',1,3),repmat('-',1,56));	
	return;
end
fprintf(fid,' %8g  %10.6e %8.2f%% %10.5f %10.5f\n',itn,r1norm, res_pct,norm1,norm2)
