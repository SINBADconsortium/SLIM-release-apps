...
if isfield(params,'d_max'), d_max = params.d_max; end
...
for i=1:length(f_a)
    ...
    if constraint.use_TD_bounds==1 && i==1 && isscalar(constraint.TD_LB)==0
        TD_LB_orig=constraint.TD_LB;
        TD_UB_orig=constraint.TD_UB;
    end
    if coarsen_grid %resample grid to current frequency batch
        ...
        d_stable(3)=1;
         d_stable = min (d_max,d_stable);
        [f2c,c2f,n_sub] = fine2coarse(model.n,model.d,d_stable,'cubic');
        
        if constraint.use_TD_bounds==1 && isscalar(constraint.TD_LB)==0
            [f2c_constraint] = fine2coarse([model.n(1) model.n(2)-1],model.d,d_stable,'cubic');
            constraint.TD_UB=f2c_constraint*TD_UB_orig(:);
            constraint.TD_LB=f2c_constraint*TD_LB_orig(:);
        end
        ...
        mk(mk<min_mk)=min_mk;
        if strcmp(model.unit,'m/s')==1
            [P_temp,] = setup_constraints_general_2D(constraint,1,comp_grid,mk(:));
        else
...
%             funProj = @(input) P_box(1e6./(Dykstra_prox_parallel((1e3./sqrt(P_box(input))),Projs,constraint.options_dyk).^2));
        case 'dykstra_ssq_constraints_mixed' %dykstra runs in s^2/m^2 constraints which are defined in velocity will be converted and back when projected (this is a slight deviation from the theory, often works well in practice)
            %Projs = setup_constraints_freq_FWI_2D_advanced(constraint,model,comp_grid,params,mk,i);
            [Projs,P_sub,TD_OP] = setup_constraints_freq_FWI_2D(constraint,model,comp_grid,params,mk,i);
           
            funProj = @(input) Dykstra_cyclic(input,Projs,P_sub,TD_OP,constraint.options_dyk);
            if use_POCS==1
        ...
        case 'single_constraint'
            if strcmp(model.unit,'m/s')==1
                    [Projs] = setup_constraints_general_2D(constraint,1,comp_grid,mk(:));
                else
        ...
        case 'SDMM'
            [Projs,P_sub,TD_OP] = setup_constraints_freq_FWI_2D_advanced(constraint,model,comp_grid,params,mk,i);
            funProj = @(input) Compute_projection_intersection_ARSDMM(input,TD_OP,P_sub,constraint.ADMM_options);
        case 'Minkowski'
            [Projs,P_sub,TD_OP] = setup_constraints_general_2D(constraint,1,comp_grid,mk(:));
            funProj = @(input) Compute_projection_Minkowski(input,TD_OP,P_sub,constraint.ADMM_options);
        otherwise
