function varargout = PDEfunc_wri_srcest2(func,m,Q,dm,D,model,params)
...
Alpha = m(prod(model.n)+1:end);
m         = m(1:prod(model.n));
Alpha = Alpha(params.i_freq);
...
if isfield(params,'SrcEst')
    SrcEst = params.SrcEst;%disp('ok')
else
    SrcEst = 0;%disp('not ok')
end
est_src = [];
...
for k=1:length(freq)
    ...
    if isfield(params,'WR')
        if isfield(params, 'SIGMA_h')
            Dc   = params.SIGMA_h * params.WR*D(:,:,k);
            Pc    = params.SIGMA_h*  params.WR*Prsp;
            S       = [lambda*Qk; Dc];
        else
            Pc   =  params.WR * Prsp;
            Dc   = params.WR*D(:,:,k)
        ...
        if isfield(params, 'SIGMA_h')
            Pc    = params.SIGMA_h * Prsp;
            Dc   = params.SIGMA_h * D(:,:,k);
            S       = [lambda*Qk;Dc];
        else
            Pc    = Prsp;
            Dc   = D(:,:,k);
    ...
    if SrcEst > 0
        ...
        if isfield(params,'SolSrc')
            SolSrc = params.SolSrc;
        else
            SolSrc = 1;
        end
        if SolSrc == 1
            [U,est_src_t] = Solve_WRI_SrcEst(Hk, Pc, Qkt, Dc, lambda);
        else
            [U,est_src_t] = Solve_WRI_SrcEst2(Hk, Pc, Qkt, Dc, lambda);
        end
        est_src = [est_src est_src_t(:)];
        
        
        Qkts = Qkt;
        for iq = 1: length(est_src_t)
            Qkts(:,iq) = Qkts(:,iq) * est_src_t(iq);
        end
        
        
        pde_res = Hk * U - Qkts;
        %       pde_res = Hk * U - (opDiag(conj(est_src_t))*Qkt')';
        
        %         for iu = 1:size(Qkt,2)
        %                 nqc            = size(Qk,1);
        %                 s_iu            = S(:,iu);
        %                 Aq_iu         = zeros(size(A,1),1);
        %                 Aq_iu(1:nqc) = -s_iu(1:nqc);
        %                 s_iu(1:nqc)     = 0;
        %                 A_iu                   = [A Aq_iu];
        %                 U_iu                       = A_iu \ s_iu;
        %                 U(:,iu)                    = U_iu(1:end-1);
        %                 est_src(iu,k)        =  U_iu(end) * w(k);
        %
        %                 pde_res(:,iu)       = Hk*U(:,iu) - U_iu(end)*Qkt(:,iu);
    ...
    switch func
        case OBJ
            ...
            if isfield(model,'sigma')
                ...
                g_alpha(k) = lambda^2/sigma_k^2 * sum(vec(conj(Qks) .* (-pde_res)) );
            else
                g     = g + lambda^2 * Pcomp * (sum(real(B1'*(V1.*V2)),2));
                g_alpha(k) = lambda^2 * sum(vec(conj(Qks) .* (-pde_res)) );
            end
...
if SrcEst > 0
    f_aux.est_src = est_src;
end

if params.FlagUandAlpha > 0
    f_aux.gest_src = g_alpha;
end
