function [f g H w] = misfit_uq_slowness_srcest2(m,D,Q,model,params);
...
%gmis(prod(model.n)+1:end)


[fpr gpr Hpr]    = fhp(m(1:prod(model.n)));

f    = fmis + beta * fpr;
g   = gmis;
g(1:prod(model.n))    = gmis(1:prod(model.n)) + beta * gpr;
H    = Hmis + beta * Hpr;
...
if model.Priormp > 0
    ...
    f        = f + fpm;
    g(1:prod(model.n))       = g(1:prod(model.n)) + gpm;
    H        = H + Hpm;
    
end

%g(prod(model.n)+1:end)
%g(prod(model.n)+1:end)=0;
%g = g(1:prod(model.n));
