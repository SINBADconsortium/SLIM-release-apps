...
for k=1:nfreq
     ...
     [Hk,A,B,C,Dc]  = Helm2D(fm,ot,dt,nt,model.nb);
     ...
     switch func,  
         ...
         case OBJ
             ...
             if nargout >= 2
                ...
                for iu = 1: size(Uk,2)
                    ...
                    gi = real(Pcomp*( conj(((Dc.*spdiags([ui,ui,ui],[-1 0 1], Nt, Nt)) * spdiags(df, Nt, Nt)))  * Vk(:,iu) ) );
                    g  = g + gi;
                end
         ...
         case JACOB_FORW
             ...
             for iu = 1:size(Uk,2)
                 ui  = Uk(:,iu);
                 JSi = (Dc.*spdiags([ui,ui,ui],[-1 0 1], Nt, Nt)) * (df .* input );
                 JS(:,iu)  = JS(:,iu) - JSi;
             end
         ...
         case JACOB_ADJ        
             Vk = Hinvadj(-Pr'*reshape(input(:,k),nrec,nsrc));
             r = sum( conj(dM * Uk) .* (A'*Vk) ,2 );
             
             for iu = 1 : size(Uk,2)
                 
                 ui = Uk(:,iu);
                 ri = real(conj(Dc.*spdiags([ui,ui,ui],[-1 0 1], Nt, Nt) * spdiags(df, Nt, Nt)) * Vk(:,iu));
                 r  = r + ri;
