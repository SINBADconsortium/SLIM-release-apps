function [H,A,Bk,C,Dc] = Helm2D(k,o,d,n,nb)
...
rc1(Ia1,:) = 0;
rc2(Ia1,:) = 0;
dc1(Ia1,2) = -1i * (1 + 1i * k(Ia1') * d(1)).^(-2) / d(1);
...
rc1(Ib1,:) = 0;
rc2(Ib1,:) = 0;
dc1(Ib1,2) = -1i * (1 + 1i * k(Ib1') * d(1)).^(-2) / d(1);

% left
c2(Ia2,2) = c2(Ia2,2) + (1./(1+1i*k(Ia2')*d(2)))/d(2)^2;
rc1(Ia2,:) = 0;
rc2(Ia2,:) = 0;
dc2(Ia2,2) = -1i * (1 + 1i * k(Ia2') * d(2)).^(-2) / d(2);

% right
c2(Ib2,2) = c2(Ib2,2) + (1./(1+1i*k(Ib2')*d(2)))/d(2)^2;
rc1(Ib2,:) = 0;
rc2(Ib2,:) = 0;
dc2(Ib2,2) = -1i * (1 + 1i * k(Ib2') * d(2)).^(-2) / d(2);
...
H  = A*E + C;
Dc = alpha * (spdiags(dc1,[-1 0 1],N,N) + spdiags(dc2,[-n(1) 0 n(1)],N,N));
