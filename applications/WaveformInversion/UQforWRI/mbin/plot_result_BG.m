ResDir = '/scratch/zfang/Result/UQWRI/BGModel/Journal/FinalResult_20';
...
cax_M       = [1.5 4.5];
cax_STD     = [0.0074 1];
CAX         = {cax_M, cax_M,[0.0074,1], cax_M, cax_M,     cax_STD, cax_M,   cax_STD};
Paper_Model = [10 4];
...
%        plot_MeanCmp(zz,velRTO,stdRTO,velINV,stdINV);
        [l p] = plot_MeanCmp(zz,velRTO,stdRTO,velmp,stdmp);
        xlabel('Velocity [km/s]','fontsize',20)
        ylabel('Depth [m]','fontsize',20)
        hold on;
        ht  = plot(velmt,zz,'linewidth',2,'color','k');
        %set(ht,'Ydir','reverse');
        h   = legend([p(1) p(2) ht(1)],'RTO result','Prior model','True model');
...
if FlagPrior > 0
    ...
    for i = 1:length(I)
        ...
        figure;fidx=gcf;
%        plot_MeanCmp(zz,velRTO,stdRTO,velINV,stdINV);
        [l p] = plot_MeanCmp(zz,velmp,stdmp,velmp,stdmp);
        xlabel('Velocity [km/s]','fontsize',20)
        ylabel('Depth [m]','fontsize',20)
        hold on;
        ht  = plot(velmt,zz,'linewidth',2,'color','k');
        %set(ht,'Ydir','reverse');
        h   = legend([p(2) ht(1)],'Prior model','True model');
        set(h,'fontsize',14);set(gca,'fontsize',20);
        filename = [FigDir '/BG_STDPriorX_' num2str(xpos(i))];
        PrintFigure(fidx,Paper_STDcmp,filename);pause(0.5)
        convert_command = ['convert ' filename '.pdf ' filename '.png'];
        status          = unix(convert_command);
    end
