...
%
% USE:
%   [H, A] = helmholtz_solver(model,freq,par_helm,par_solver,flog,sep_grids,H)
%
% INPUT:
...
%  sep_grid   - if this flag is set to 1, each PDE will be discretized and
%               solved on the grid and pml designed for that frequency.
% H           - [OPTIONAL] in case a PDE discretization is provided, you
%               can use this code just to solve the problem by providing H.
%
% OUTPUT:
%           H - A structure containing at least H.coeff (the discrete operator
%               in bandstorage formate, e.g. the output of helmholtz_3d) and
...
%               equation after, according to the parameters chosen during the
%               creation of this function.
%               H contains all the fields from the output of the function
%               discrete_helmholtz; please refer to "help discrete_helmholtz"
...
% Date: January, 2014
%
% Adapted to 2D problems by Bas Peters, March 2015
%
...
% 
%------------------------------------------------------------------------------- 
function [H, A] = helmholtz_solver_2D(model,freq,par_helm,par_solver,flog,sep_grids,H)

% These are the available solves
% (well, sort of; this needs to be properly implemented)
%-------------------------------------------------------------------------------

solve_name = check_field(par_solver,'name','');

%% TODO: Allow a different solve for forward and for the adjoint solve - Lago
use_carpcg = 0;
use_carpcr = 0;
use_backslash = 0;
use_mlcrmn = 0;
...
if nargin> 5
    if sep_grids==1
    stable = discrete_helmholtz_2D(model,freq,...
        setfield(par_helm,'discretize',false));
    else
        [R,idx] = mat2R(H); clear H; H.coef=R.'; H.idx=idx; clear idx
        H.nt = model.n+2*model.nb;
    end
end
%-------------------------------------------------------------------------------
% AUTO
