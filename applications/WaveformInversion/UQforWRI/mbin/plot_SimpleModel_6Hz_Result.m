ResDir = '/scratch/zfang/Result/UQWRI/SimpleLayer/Journal/RTO_SMP_6Freq';
...
STD_SMP_INV_FILE  = [ResDir '/STD_INV.mat'];
MEAN_SMP_INV_FILE = [ResDir '/MEAN_INV.mat'];
...
str_Model   = {'Layer_True','Layer_Ini','Layer_Inistd','Layer_MAP','Layer_RTOMEAN','Layer_RTOSTD','Layer_INVMEAN','Layer_INVSTD'};
M_Model     = {m_true,m_ini,SIGMA_mp,m_MAP, MEAN_RTO, STD_RTO, MEAN_INV, STD_INV};
C_MAP       = {jet,   jet,  redblue, jet,   jet,      redblue, jet,      redblue};
cax_M       = [1.5 2.5];
cax_STD     = [0.003 0.25];
CAX         = {cax_M, cax_M,cax_STD, cax_M, cax_M,     cax_STD, cax_M,   cax_STD};
Paper_Model = [10 4];
...
if FlagModel > 0
    ...
    for i = 1:length(str_Model)
        filename = [FigDir '/' str_Model{i} '_6Freq'];
        mtmp     = M_Model{i};
        CMAPtmp  = C_MAP{i};
        mtmp(1,:)   = mtmp(2,:);
        mtmp(end,:) = mtmp(end-1,:);
...
if FlagCross > 0
    ...
    for i = 1:length(I)
        ...
        h   = legend('Approach 3','Approach 1');
        set(h,'fontsize',14);set(gca,'fontsize',20);
        filename = [FigDir '/Layer_STDcmpX_' num2str(xpos(i)) '_6Freq'];
