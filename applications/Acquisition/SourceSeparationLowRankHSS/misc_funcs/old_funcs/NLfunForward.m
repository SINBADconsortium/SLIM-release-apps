...
if isempty(g)
    f1 = params.afun([vec(L1*R1');vec(L2*R2')]);
    f2 = 0;
else 
    fp1 = params.afunT(g); % A'(r1), where r1 is the residual
    ...
    f1 = [vec(fps1*R1); vec(fps1'*L1);vec(fps2*R2); vec(fps2'*L2)]; % gradient wrt L1 , R1 , L2 , R2
    f2 = [vec(fp1)];
end
