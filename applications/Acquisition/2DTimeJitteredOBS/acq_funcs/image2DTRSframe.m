function image2DTRSframe(fname, frame, nt, dt, caxmin, caxmax, cmap)
...
% Time slice
figure
imagesc(real(squeeze(data(frame.t,:,:))), [-caxmin caxmax]); colormap(cmap); 
set(gca,'plotboxaspectratio',[1 1 1]); colorbar;
xlabel('Shot number'); ylabel('Receiver number'); title('Time slice')

% Receiver gather
figure
imagesc(real(squeeze(data(:,frame.r,:))), [-caxmin caxmax]); colormap(cmap); 
...
% Shot gather
figure
imagesc(real(squeeze(data(:,:,frame.s))), [-caxmin caxmax]); colormap(cmap); 
set(gca,'plotboxaspectratio',[1.5 2 1.5]); colorbar;
