function jitacq = jitter_airgunarrays(ns, ds, dt, p, nboats, randseed, boatspeed, tfireint_min, tdelay, delayboat, fig)
...
%         randseed - random seed [NOTE: set two seeds for one boat (with two airgun arrays), and four seeds for two 
...
sjitb1arr1 = round(boatspeed*tfirejitb1arr1);  
gridsjitb1arr1IND = floor(sjitb1arr1/ds);
...
rng(randseed(2));
dtfirejitb1arr2 = tfireint_min + rand(1,round(ns/p))*(2*tfireint_min);
tfirejitb1arr2 = cumsum(dtfirejitb1arr2); 
sjitb1arr2 = round(boatspeed*tfirejitb1arr2); 
gridsjitb1arr2IND = floor(sjitb1arr2/ds);
...
if nboats == 2
   ...
   sjitb2arr1 = round(boatspeed*tfirejitb2arr1);  
   gridsjitb2arr1IND = floor(sjitb2arr1/ds);
   ...
   rng(randseed(4));
   dtfirejitb2arr2 = tfireint_min + rand(1,round(ns/p))*(2*tfireint_min);
   tfirejitb2arr2 = cumsum(dtfirejitb2arr2); 
   sjitb2arr2 = round(boatspeed*tfirejitb2arr2); 
   gridsjitb2arr2IND = floor(sjitb2arr2/ds);
...
if nboats == 1
   tfirejitgrid = 0 : dt : tfirejitb1arr2(end);
...
elseif nboats == 2
   tfirejitgrid = 0 : dt : tfirejitb2arr2(end);
   tfirejitb1arr1gridIND = zeros(1,length(tfirejitb1arr1)); 
   tfirejitb1arr2gridIND = zeros(1,length(tfirejitb1arr2)); 
...
if strcmp(fig, 'yes')
   if nboats == 1
      figure
      plot(sjitb1arr1, tfirejitb1arr1, 'o', sjitb1arr2, tfirejitb1arr2, 'b*'); legend('Array 1', 'Array 2'); grid on; axis ij, axis('tight'); 
      xlabel('Source position (m)'); ylabel('Recording time (s)'); title('Time-jittered acquisition with one source vessel')
   elseif nboats == 2
      figure
      plot(sjitb1arr1, tfirejitb1arr1, 'o', sjitb1arr2, tfirejitb1arr2, 'b*', sjitb2arr1, tfirejitb2arr1, 'bo', sjitb2arr2, tfirejitb2arr2, 'b*'); 
      legend('Array 1', 'Array 2'); grid on; axis ij, axis('tight'); 
      xlabel('Source position (m)'); ylabel('Recording time (s)'); title('Time-jittered acquisition with two source vessels')
   end
end
