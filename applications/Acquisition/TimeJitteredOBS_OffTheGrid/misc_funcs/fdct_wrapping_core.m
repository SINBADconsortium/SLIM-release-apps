function C = fdct_wrapping_core(X, nbscales, nbangles_coarse)

% fdct_wrapping.m - Fast Discrete Curvelet Transform via wedge wrapping - Version 1.0
%
% Inputs
%   X           M-by-N matrix in the Fourier domain
%
% Optional Inputs
...
%                   and l.
%
% See also ifdct_wrapping_core.m
%
% Original code (fdct_wrapping.m) by Laurent Demanet, 2004
% Modified by Gilles Hennenfent, 2008
%   - input matrix is in the Fourier domain
%     ( as opposed to computing FFT2 in the original code: X = fftshift(fft2(ifftshift(x)))/sqrt(prod(size(x))) )
%   - type of transform is set to complex (is_real = 0)
%   - coefficients at the finest level are set to curvelets (finest  = 1)
...
[N1,N2] = size(X);
if nargin < 2, nbscales = ceil(log2(min(N1,N2)) - 3); end;
if nargin < 3, nbangles_coarse = 16; end;
