...
% the predicted data for the given acquisition and model.
% 
% This is nothing but a wrapper to WaveEquationFD function
%
% Use: 
%   d_pred = ForwardFD(model, acq, opts, flog)
...
% 
% IMPORTANT: The outputed data will have the same dimension
...
% software.
%-------------------------------------------------------------------------------
function d_pred = ForwardFD(model, acq, opts, flog)
   
   if ~exist('flog','var')
      flog = 0;
   end
   
   opts.output_d_pred = 1;
   opts.output_misfit = 0;
   opts.output_grad   = 0;
   [~, ~, d_pred] = WaveEquationFD(model, acq, opts, flog);
