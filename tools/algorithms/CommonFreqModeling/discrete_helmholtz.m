...
%
% USE:
%   [H,comp_grid] = discrete_helmholtz(v,model,freq,opts,flog)
%
% INPUT:
...
%
%-------------------------------------------------------------------------------
function [H,comp_grid,dH,ddH] = discrete_helmholtz(v,model,freq,opts)
...
if mat_free         
    ...
    switch scheme
      ...
      case PDEopts.HELM2D_CHEN9P                
        [H,dH,ddH] = Helm2D_opt(v_pml,dt,nt,npml(:,1:2),model.unit,freq,model.f0);  
        helm_params = struct;
        helm_params.coef = H;
