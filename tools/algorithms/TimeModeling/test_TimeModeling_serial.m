function test_TimeModeling_serial()
...
disp('perform dottest on op_Src_Rcv_intep');
dottest(Pr,1,1);
...
disp('perform dottest on Jacobian of 2D time domain modeling kernel');
...
disp('perform dottest on Jacobian of 3D time domain modeling kernel');
disp(['dottest, difference is ',num2str(gather(a)-b),'; ratio is ', num2str(gather(a)/b)]);
elapsedtime=toc(starttime);;
	fprintf('PASSED! (in %.1f seconds)\n',elapsedtime);
catch ME
	fprintf('%s\nFAILED!\n',ME.message);
end
end
