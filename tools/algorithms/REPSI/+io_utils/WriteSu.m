...
if exist('dt')~=1
  dt=0.004;
  io_utils.SegymatVerbose([mfilename,' - No dt set. Using dt=',num2str(dt)])
end
...
SegyHeader.Rev=io_utils.GetSegyHeaderBasics;
...
SegyTraceHeader=io_utils.InitSegyTraceHeader(ns,dt*1e+6);
for i=1:ntraces;
    if (i/100)==round(i/100), io_utils.SegymatVerbose(['writing trace ',num2str(i),' of ',num2str(ntraces)],1),end
    ...
    if exist('FieldRecord')==1,SegyTraceHeader.FieldRecord=FieldRecord(i);end
    if exist('TraceNumber')==1,SegyTraceHeader.TraceNumber=TraceNumber(i);end 
    if exist('DelayRecordingTime')==1,SegyTraceHeader.DelayRecordingTime=DelayRecordingTime(i);end 
    % Write the Trace
    io_utils.PutSegyTrace(segyid,data(:,i),SegyTraceHeader,SegyHeader);
end
