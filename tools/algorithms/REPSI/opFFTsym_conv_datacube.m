function op = opFFTsym_conv_datacube(nt, ns, nr, mask, compress_size, precision)
% OPFFTSYM_CONV_DATACUBE  	 One-dimensional symmetric real fast Fourier transform (FFT).
%                        	 NOTE: Specialized version adapted for EPSI_SLIM
% 
%    Does not include self-adjoint scaling and does not double the magnitude of the 
%    positive frequencies. WILL NOT PASS DOT-TEST.
...
%
%
%    opFFTsym_conv_datacube(nt, ns, nr) creates a one-dimensional
%    normalized Fourier transform along the first axis of Greens
%    function datacube
%
%    opFFTsym_conv_datacube(nt, nr nr, mask) if mask is a 1D vector
...
%    matrix in frequency defined by mask as an array of indicies, 1 where
%    the frequencies are selected and 0 where the frequencies do not exist
%    (note: need length(mask) <= nf, and max(mask) <= nf)
%
%    opFFTsym_conv_datacube(nt, ns, nr, mask) if mask is a 2D vector
%    creates a one-dimensional normalized Fourier transform along the
...
%
%    opFFTsym_conv_datacube(nt, ns, nr, mask, compress_size) if the
...
if ~exist('mask','var')
    ...
    subfunc_handle = @(x,mode) opFFTsym_conv_datacube_intrnl(nt,ns,nr,x,mode);
    ...
    subfunc_handle = @(x,mode) opFFTsym_conv_datacube_intrnl(nt,ns,nr,x,mode,mask,compress_size);
    op = opFunction(mft,n,subfunc_handle); % return a SPOT operator using constructor opFunction
end


function y = opFFTsym_conv_datacube_intrnl(nt,ns,nr,x,mode,mask,compress_size)
    ...
    if mode == 0
       ...
       if ~exist('mask','var')
            % no mask defined
            y = {m,n,[0,0,0,0],{'FFTsym_conv_datacube'}};
       else
            y = {mft,n,[0,0,0,0],{'FFTsym_conv_datacube with frequency restrictions'}};
       end
