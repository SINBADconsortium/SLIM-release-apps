...
function test_opKron_Diag
rng('default');

A1 = randn(3,4) + 1i*randn(3,4);
A2 = randn(3,2) + 1i*randn(3,2);
A3 = randn(2,1) + 1i*randn(2,1);

A  = kron(A1,kron(A2,diag(A3)));
B  = kron(opMatrix(A1),kron(opMatrix(A2),opDiag(A3)));
C  = kron(opMatrix(A1),opMatrix(A2),opDiag(A3));
