function [ x, itn, r1norm, istop,r2norm, Anorm, Acond, Arnorm, xnorm, var ]...
    = lsqrSOL( A, b, opt )
...
if isa(A,'numeric')
    explicitA = true;
        ...
        explicitA = false;
    else if isa(A,'opSpot')
            explicitA = true;
        else
            error('SOL:lsqrSOL:Atype','%s','A must be numeric or a function handle or a spot operator');
        end
    end
end
...
if show
    fprintf(fid,' ');
    fprintf(fid,'LSQR            Least-squares solution of  Ax = b\n');
    str1 = sprintf('The matrix A has %8g rows  and %8g cols\n', m,n);
    str2 = sprintf('damp = %20.14e    wantvar = %8g\n', damp,wantvar);
    str3 = sprintf('atol = %8.2e                 conlim = %8.2e\n', atol,conlim);
    str4 = sprintf('btol = %8.2e                 itnlim = %8g\n'  , btol,itnlim);
    fprintf(fid,str1);   fprintf(fid,str2);   fprintf(fid,str3);   fprintf(fid,str4);
end
...
if show
    fprintf(fid,'\nlsqrSOL finished\n');
    fprintf(fid,msg(istop+1,:));
    fprintf(fid,'\n');
    str1 = sprintf( 'istop =%8g   r1norm =%8.1e\n',   istop, r1norm );
    str2 = sprintf( 'Anorm =%8.1e   Arnorm =%8.1e\n', Anorm, Arnorm );
    str3 = sprintf( 'itn   =%8g   r2norm =%8.1e\n',     itn, r2norm );
    str4 = sprintf( 'Acond =%8.1e   xnorm  =%8.1e\n', Acond, xnorm  );
    fprintf(fid,[str1 str2]);
    fprintf(fid,[str3 str4]);
    fprintf(fid,'\n ');
end
