function [x,fsave,funEvals,iter_save] = minConf_PQN(funObj,x,funProj,options)
% function [x,f] = minConf_PQN(funObj,x,funProj,options)
%
...
[verbose,numDiff,optTol,progTol,maxIter,maxProject,suffDec,corrections,adjustStep,bbInit,...
    SPGoptTol,SPGprogTol,SPGiters,SPGtestOpt,fprogTol] = ...
    myProcessOptions(...
    options,'verbose',2,'numDiff',0,'optTol',1e-5,'progTol',1e-9,'maxIter',15,'maxProject',100000,'suffDec',1e-4,...
    'corrections',10,'adjustStep',0,'bbInit',0,'SPGoptTol',1e-6,'SPGprogTol',1e-10,'SPGiters',10,'SPGtestOpt',0,'fprogTol',1e-5);
...
while funEvals <= maxIter
    ...
    if abs(f-f_old) < fprogTol*abs(f) && i > 1
        if verbose >= 1
            fprintf('Function value changing by less than fprogTol\n');
        end
        break;
    end
