function [x,f,funEvals,projects] = minConf_SPGTS(funObj,x,funProj,options)
...
if testOpt && testInit
    ...
    if norm(funProj(x-g)-x,optNorm) < optTol
        if verbose >= 1
        fprintf('First-Order Optimality Conditions Below optTol at Initial Point, norm g is \n', norm(g));
        end
        return;
    end
end
...
while funEvals <= maxIter
    ...
    if ~curvilinear
		if i==1
			d = funProj(x+alphaG/max(abs(d))*d)-x;
		else
		d = funProj(x+d)-x;
		end
        projects = projects+1;
	...
	if isfield(options,'saveloc')
		if options.saveloc
			save([options.name num2str(i) '.mat'],'x','g','f');
		end
	end
    % Output Log
    if verbose >= 2
