function [x,fsave,funEvals,err,iter_save,misfit_aux] = minConf_TMP(funObj,x,LB,UB,options)
...
[verbose,numDiff,optTol,maxIter,suffDec,interp,method,corrections,damped,obj_aux,true_sol] = ...
    myProcessOptions(...
    options,'verbose',3,'numDiff',0,'optTol',1e-6,'maxIter',500,'suffDec',1e-4,...
    'interp',1,'method','lbfgs','corrections',100,'damped',0,'obj_aux',0,'true_sol',[]);
if ~isfield(options,'mode'), options.mode = ''; end
% Output Log
if verbose >= 3
...
while funEvals <= maxIter
    ...
    if obj_aux
    if options.wri==1
        misfit_aux(i,:)=[f_aux.pde f_aux.dat 0];
    else
        misfit_aux(i,:)=[0 0 0];
    end
    end
