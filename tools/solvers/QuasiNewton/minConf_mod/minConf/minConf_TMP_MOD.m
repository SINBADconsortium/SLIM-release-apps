function [x,fsave,funEvals,err,iter_save,misfit_aux] = minConF_BC_MOD(funObj,x,LB,UB,options,model)
...
while funEvals <= maxIter
    ...
    switch(method)
		...
		posDef = 0;
            if posDef == 0
                %d(working) = -R\(R'\g(working)); -> replace this with my own  routine for reduced-space Newton-type step computation
                H=(H);
    		g=(g);
    		pr = ssbin(H(working,working),500);
    		L=spdiags(pr,0,length(pr),length(pr));
    		d(working)=real(L*(L*H(working,working)*L\(-L*(g(working)))));
            %[d(working),flag,relres,iter,resvec] = pcg(L*H(working,working)*L,(-L*(g(working))),[],50,[],[],[]);
            %d(working)=L*d(working);
            else
                if verbose == 3
    ...
    if secondOrder
        [f_new,g_new,H,~,f_aux] = funObj(x_new);
    else
        [f_new,g_new,~,~,f_aux] = funObj(x_new);
    end
    funEvals = funEvals+1;
    %[norm(g_new(1:length(g_new)/2)) norm(g_new(1+length(g_new)/2:end))]
    ...
    f = f_new;
    fsave = [fsave ; f];
    misfit_aux(i,:)=[f_aux.pde f_aux.dat f_aux.reg];
    
    %err = [err norm(x-options.mtrue)];
    %save errors
    kappap_est = x(1:prod(model.n));
    bp_est     = x(1+prod(model.n):end);
    vp_est     = sqrt(1./(1e-6*kappap_est.*(1./bp_est)));
    error.t=norm(options.mtrue-x);
    error.k=norm(options.kappatrue-kappap_est);
    error.b=norm(options.btrue-bp_est);
    error.v=norm(options.vtrue-vp_est);
    err{i}=error;
