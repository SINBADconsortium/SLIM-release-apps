function [x,f,hst,funEvals] = minConf_PQNTS(funObj,x,funProj,options)
...
% Output Log
if verbose >= 2
        fprintf('%10s %10s %10s %15s %15s %15s %15s\n','Iteration','FunEvals','Projections','Step Length','Function Val','Norm d','Opt Cond');
end
...
i = 1;
%
fprintf('%10d %10d %10d %15.5e %15.5e %15.5e %15.5e\n',0,funEvals*funEvalMultiplier,projects,0,f,norm(g),0);
while funEvals <= maxIter

    % Compute Step Direction
    if i == 1 || resetH
        p = funProj(x-alphaG/max(abs(g(:)))*g);
        projects = projects+1;
        S = zeros(nVars,0);
        Y = zeros(nVars,0);
        Hdiag = 1;
    ...
    d = p-x;
	% if i>1
	% 	save([options.name num2str(i) 'before.mat'],'g_old','g','d','p','x_old','x');
	% end
    g_old = g;
    x_old = x;
    ...
    if i == 1 || adjustStep == 0
       t = 1; 
        ...
        t =min(1, alphaG/max(abs(d)));%min(1,2*(f-f_old)/gtd);
		if isinf(t) || isnan(t)
			t=1;
		end
    end
	...
	%rescale(g_new,-alphaG,alphaG);
    funEvals = funEvals+1;
    % Backtracking Line Search
    f_old = f;
    if verbose >= 2
            fprintf('Decrease condition :%15.5e %15.5e %15.5e\n',t,f_new,f + suffDec*d'*(x_new-x));
    end
	nlinsearch=0;
    while f_new > f - suffDec*d'*(x_new-x) || ~isLegal(f_new) || ~isLegal(g_new)
        temp = t;
        ...
        [f_new,g_new] = funObj(x_new);
        funEvals = funEvals+1;
		nlinsearch=nlinsearch+1;
		if nlinsearch>5
			 fprintf('To many linesearch iteration\n');
			 x_new=x;
			 f_new=f;
			 g_new=g;
			 resetH=1;
    ...
    optCond = max(abs(funProj(x+t*d)-x));
    projects = projects+1;
    hst=[hst f];
	
	if isfield(options,'name')
		save([options.name num2str(i) '.mat'],'x','g','d');
	end

    % Output Log
    if verbose >= 2
            fprintf('%10d %10d %10d %15.5e %15.5e %15.5e %15.5e\n',i,funEvals*funEvalMultiplier,projects,t,f,norm(d),optCond);
    end

    % Check optimality
    if optCond < optTol && i>1
        if verbose >= 1
    ...
    if max(abs(t*d)) < progTol && i>1
        if verbose >= 1
            fprintf('Step size below progTol\n');
        end
        resetH=1;
		fprintf('Reset history\n');
    end

    if abs(f-f_old) < progTol && i>1
        if verbose >= 1
...
[p,f,funEvals,subProjects] = minConf_SPGTS(funObj,x,funProj,options);
end
