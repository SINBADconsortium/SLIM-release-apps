function [x,fsave,funEvals,projects,iter_save] = minConf_SPG(funObj,x,funProj,options)
...
%       testOpt: test optimality condition (default: 0)
...
%
% Modified from original by Bas Peters.
...
[verbose,numDiff,optTol,progTol,maxIter,suffDec,interp,memory,useSpectral,curvilinear,feasibleInit,testOpt,bbType,testEvol,evol_tol,fprogTol] = ...
    myProcessOptions(...
    options,'verbose',2,'numDiff',0,'optTol',1e-5,'progTol',1e-9,'maxIter',15,'suffDec',1e-4,...
    'interp',2,'memory',5,'useSpectral',1,'curvilinear',0,'feasibleInit',0,...
    'testOpt',0,'bbType',1,'testEvol',1,'evol_tol',1e-4,'fprogTol',1e-4);
...
t = t/2;
    ...
    %check for sufficient evolution
    if testEvol
        if ((norm(x_new-x)/norm(x) < evol_tol) && (lineSearchIters<3) && (i>3)) && (verbose >= 2)
            fprintf('change in x is too small, exit SPG\n');
            break
        end
    end
    ...
    if abs(f-f_old) < fprogTol*abs(f) && i > 1
        if verbose >= 1
