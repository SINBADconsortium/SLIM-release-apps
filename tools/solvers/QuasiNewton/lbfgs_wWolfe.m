function [x] = lbfgs_wWolfe(fh,x0,options)
...
while ~converged
    ...
    % linesearch
    [ft,gt,lambda,lsiter] = wWolfeLS(fh,x,f,g,s,init_step);
...
function [ft,gt,lambda,lsiter] = wWolfeLS(fh,x0,f0,g0,s0,init_step)
...
lambda = .5;
if init_step
    lambda = 0.1*lambda*norm(x0)/norm(s0);
end
while ~done
    if nu < inf
        lambda = (nu + mu)/2;
