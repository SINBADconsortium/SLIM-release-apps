classdef opHTuckJ < opSpot
% OPHTUCKJ - Jacobian of the function the full-tensor expansion operator 
...
%                      it is computed in this method) 
    ...
    methods        
        function op = opHTuckJ(dimTree,x, fullTree)
            n = prod(dimTree.dims);
            ...
            if(mode == 1)
                ...
                while itr.advance()
                   ...
                   if itr.isRoot()
                       ...
                       dUR = opProjection(UR,true,true)* ( ((UL * B)' * dU ) .' ) ;
                       dB = UL' * dUL1;    
                       clear dUL1 x;
                   else
                       dU = dematricize(dU,[nl, nr, k],[1 2]);
                       dUL = opProjection(UL,true,true) * (ttt(B,ttm(dU,UR',2),[2 3],[2 3]) .');
                       dUR = opProjection(UR,true,true) * (ttt(B,ttm(dU,UL',1),[1 3],[1 3]) .');
