...
% argmin_u || [lambda*H ; P]u - [lambda*Q;D] ||_2
...
if exist('params','var') && ~isempty(params)
    if isfield(params,'analysis'), analysis = params.analysis; end
    if isfield(params,'sim_rec'),  sim_rec  = params.sim_rec; end
    if isfield(params,'n_sim_rec'),  n_sim_rec  = params.n_sim_rec; end
    if isfield(params,'refine'),   refine   = params.refine; end
    if isfield(params,'maxit_refine'), maxit_refine = params.maxit_refine; end
end
...
%% Refine solutions
% use the result so far as initial guess for unpreconditioned LSQR for the
% orignial problem
if refine==1
    tic %timer for solution refinement process
    %define matrix-vector product with [lambda*H;P]
    MVP=@(in,mode) MVP_data_aug_syst(in,mode,P,H,lambda);
    
    for i=1:size(Q,2);
        [Uk(:,i), flag, relres, it ,rv] = lsqr(MVP,[lambda*Q(:,i);D(:,i)],10*eps,maxit_refine,[],[],Uk(:,i));
    end
    timing.refine=toc;
end
