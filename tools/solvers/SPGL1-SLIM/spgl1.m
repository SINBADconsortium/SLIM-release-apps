...
% 20 Feb 13: Branched into Git for more changes
% 31 Mar 13: Improved memory usage by agressive inlining of several functions, as well as changing how prevG is obtained
% 04 Apr 13: Feasibility of norm contraint is now compared relatively instead of absolutely
% 09 Apr 13: Removed superflous Kaczmarz support
% 09 Apr 13: Disabled projected line search for first iteration if x0 is empty (step-length scaling is likely to be very wrong)
...
% maxLineErrors TEMPORARILY DISABLED to prevent very large scaling issue from throwing off algorithm
...
explicit = not(isa(A,'function_handle'));
if isempty(x)
   if explicit
      n = size(A,2);
   ...
   % The first gradient length is expected to be very wrong
   doFirstIterProjectedLineSrch = false;
else
   n     = length(x);
   realx = isreal(x) && isreal(b);
   if norm(x) == 0
       % empty but pre-initialized x
       doFirstIterProjectedLineSrch = false;
   else
       doFirstIterProjectedLineSrch = true;
   end
end
if explicit, realx = realx && isreal(A); end;
...
printf('\n');
printf(' %s\n',repmat('=',1,82));
printf(' SPGL1_SLIM v.%s (%s) based on original SPGL1 v.1017\n', REVISION, DATE);
printf(' %s\n',repmat('=',1,82));
...
if isempty(x)
    r         = b;  % r = b - Ax
    ng        = Aprod(r,2);  % ng = A'r = negative gradient
    f         = norm(r)^2 / 2;
    dx        = ng;
    dx        = project(dx, tau);
else
...
while 1
    ...
    gNorm   = undist(options.dual_norm(ng,weights)); % originally options.dual_norm(-g,weights), but for true norms the sign should not matter
    rNorm   = norm(r, 2);
    ...
    try
       ...
       if doFirstIterProjectedLineSrch
       ...
       if lnErr
          ...
          if ~isempty(xOld)
              dx = project(xOld + gStep.*ng, tau) - xOld;
          else
              dx = project(gStep .* ng, tau);
          end
       ...
       % Ensure that after projection the new primal norm falls below constraint
       primNorm_x = undist(options.primal_norm(x,weights));
       targetNorm = tau+optTol;
       if isa(x,'distributed')
           classX = classUnderlying(x);
       else
           classX = class(x);
       end
       primNormIsExpected = ((primNorm_x-targetNorm)/targetNorm <= eps(classX));
       
       if not(primNormIsExpected)
            if ignorePErr
                warning('Primal norm of projected x is larger than expected...')
                disp(['   primNorm_x: ' num2str(primNorm_x)])
                disp(['   targetNorm: ' num2str(targetNorm)])
            else
                % choose not to ignore error, hand to normal assertion function
                ensure(primNormIsExpected);
            end
        end
       ...
       % BEGIN OLD SECTION FOR CALCULATING STS AND STY =================
       % if isempty(xOld)
       %     s    = x;
       % else
       %     s    = x - xOld;
       ...
       % sts  = norm(s)^2;
       % sty  = -1 * dot(s,y);
       % END OLD SECTION FOR CALCULATING STS AND STY =================
       % BEGIN NEW SECTION FOR CALCULATING STS AND STY =================
       % use the relation sty = dot(x,g) - dot(x,gOld) - dot(xOld,g) + dot(xOld,gOld)
       if isempty(xOld)
           xtgOld       = -dot(x,ng);
           xOldtgOld    = 0;
           clear ng
           ng           = Aprod(r,2); % new gradient
           xtg          = -dot(x,ng);
           xOldtg       = 0;
           
           sts  = norm(x)^2;
           sty  = xtg - xtgOld - xOldtg + xOldtgOld;
       else
           xtgOld       = -dot(x,ng);
           xOldtgOld    = -dot(xOld,ng);
           clear ng
           ng           = Aprod(r,2);
           xtg          = -dot(x,ng);
           xOldtg       = -dot(xOld,ng);
