...
% 
%------------------------------------------------------------------------------
function precond = ML_CGMN(Hk,v,comp_grid,model,freq,opts,flog)
...
lv1_coarse_solver_params.maxinnerit    = 2; % # inner iterations of FGMRES
...
opts_lv2.solve_opts.precond = LinSolveOpts.PREC_IDENTITY;
...
opts_lv3.solve_opts.precond = LinSolveOpts.PREC_IDENTITY;
opts_lv3.solve_opts.solver = LinSolveOpts.SOLVE_CRMN;

% Coarsen model
[lv1_to_lv2,~,n_lv2] = fine2coarse(comp_grid.nt_nopml,comp_grid.dt,opts_lv2.dt);
v_lv2 = lv1_to_lv2*vec(v);

[lv2_to_lv3] = fine2coarse(n_lv2,opts_lv2.dt,opts_lv3.dt);
...
[lv2_to_lv3,lv3_to_lv2] = fine2coarse(comp_grid_lv2.nt,comp_grid_lv3.nt);

par_lv2.restriction  = @(u)(lv2_to_lv3*u); 
par_lv2.prolongation = @(u)(lv3_to_lv2*u);

par_lv2.pre_smoother = @(b,x0)(linsolve(SLap_lv2,b,1,x0,lv2_smoother_maxit,lv2_smoother));
par_lv2.pst_smoother = par_lv2.pre_smoother;
par_lv2.coarse       = @(b,x0)(linsolve(SLap_lv3,b,1,x0,lv2_coarse_maxit,lv2_coarse_solver));
...
if adj_mode
   ...
   par_lv2_adj.pre_smoother = @(b,x0) linsolve(SLap_lv2,b,-1,x0,lv2_smoother_maxit,lv2_smoother); par_lv2_adj.pst_smoother = par_lv2_adj.pre_smoother;
   par_lv2_adj.coarse = @(b,x0) linsolve(SLap_lv3,b,-1,x0,lv2_coarse_maxit,lv2_coarse_solver);
...
[lv1_to_lv2,lv2_to_lv1] = fine2coarse(comp_grid.nt,comp_grid_lv2.nt);
...
par_lv1.pre_smoother = @(b,x0)(linsolve(Hk,b,1,x0,lv1_smoother_maxit,lv1_smoother));

Helm_lv2 = discrete_helmholtz(v_lv2,model,freq,opts_lv2,flog);
par_lv1.coarse       = @(b,x0)(BICGSTAB(Helm_lv2,b,x0,lv1_coarse_solver_params));
...
if adj_mode
    ...
    par_lv1_adj.pre_smoother = @(b,x0) linsolve(Hk,b,-1,x0,lv1_smoother_maxit,lv1_smoother);
    
    par_lv1_adj.coarse = @(b,x0) BICGSTAB(Helm_lv2',b,x0,lv1_coarse_solver_params_adj);
